...
/// <summary>
/// A single-use, expiring token backing a "magic link" email verification. Only the
/// hash of the token is stored; the raw value lives only in the emailed link. A token
/// is valid until it is consumed or expires.
/// </summary>
public sealed class EmailVerificationToken : BaseEntity, ITenantOwned
{
    ...
    public EmailVerificationToken(Guid tenantId, string tokenHash, DateTimeOffset expiresAtUtc)
    {
